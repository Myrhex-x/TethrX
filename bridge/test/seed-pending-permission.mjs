// Seed an ACP session that reaches a permission prompt and LEAVE it pending, so the
// app can open the session and show the approval card. Prints "PENDING <sid> <reqId>".
...
const session = await (await fetch(base + "/api/sessions", {
  method: "POST", headers: H, body: JSON.stringify({ cwd, title: "approve-demo" }),
})).json();
...
const waitForPermission = (async () => {
  ...
  for (;;) {
    ...
    while ((i = buf.indexOf("\n\n")) >= 0) {
      ...
      if (ev.kind === "permission_request") { ctrl.abort(); return ev.requestId; }
...
await fetch(base + "/api/sessions/" + session.id + "/messages", {
  ...
  body: JSON.stringify({ text: "Use the shell to run exactly: echo pending > pending.txt" }),
});

const reqId = await waitForPermission;
console.log(reqId ? `PENDING ${session.id} ${reqId}` : `NO-PERMISSION ${session.id}`);
process.exit(0);
